...
def browse_to_facesheet(demo_id, prev_name="") -> bool:
    global current_patient_name, is_it_new_login
    driver.execute_script(f"fncPatientRedirect('D','{demo_id}','0',0)")
    if prev_name == "" or is_it_new_login:
    ...
    if result_demo_id is not None and int(result_demo_id) == demo_id:
        ...
        print("Demographic ID doesn't match with facesheet")
        return False
...
def restart(very_first_time=False):
    global driver, is_it_new_login
    print("--------------> Re Logging In <---------------")
    is_it_new_login = True
    if not very_first_time:
        driver.quit()
    driver = load_driver()
...
if __name__ == "__main__":
    ...
    for index, row in df.iterrows():
        ...
        if counter_failed > 10:
            print("===> More than 10 Errors occured. Exiting <====")
            break
        ...
        try:
            if row["Status"] != "Failed" and row["Status"] != "Completed":
                ...
                # variables
                status_text = f"[Status] => Pass|Fail: [{counter_success}|{counter_failed}] Time: [{time.strftime('%H:%M:%S')}] Elapsed Time: [{str(datetime.timedelta(seconds=(time.time() - start_time)))}] Avg: [{round((time.time() - start_time) / (counter_success + counter_failed if counter_success > 0 else 1))}]"
                process_text = f"Processing record: [{index + 1}/{df.shape[0]}]. \t Patient Id: [{d_id}]"
                print(status_text)
                print(process_text)
                ...
                if wait_till_load(by_id="btnReferralPrint", timeout=300):
                    ...
                    counter_success += 1
                rest.api_add_status(status_text, process_text, str(d_id))
