    ...
    for index, row in df.iterrows():
        ...
        try:
            if row["Status"] != "Failed" and row["Status"] != "Completed":
                ...
                print(
                    f"[Status] => Pass|Fail: [{counter_success}|{counter_failed}] Time: [{time.strftime('%H:%M:%S')}] Elapsed Time: [{str(datetime.timedelta(seconds=(time.time() - start_time)))}] Avg: [{round((time.time() - start_time) / (counter_success + counter_failed if counter_success > 0 else 1))}]")
